...
/*   Updated: 2022/08/30 09:28:40 by kdemirci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_colors(t_map *m)
{
	...
	while (v.y < m->height)
	{
		...
		while (v.x < m->width)
		{
			...
			cur->color = clerp(0xFF0000, 0xF1b3fc1,
					ft_ilerp(cur->z, m->depth_min, m->depth_max));
			v.x++;
		}
		v.y++;
	}
...
{
	v.x -= (double)(mlx->map->width) / 2.0f;
	v.y -= (double)(mlx->map->height) / 2.0f;
